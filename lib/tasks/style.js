...
/**
 * Get lint config path (sass or stylus)
 * @param extention{string} style file extention '.scss' or '.styl, .stylus'
 * @returns {string}
 */
function lintConfigPath(extention) {
  let lintFile = (extention === 'scss') ? '.sass-lint.yml' : '.stylintrc';
  let configPath = path.join(process.cwd(), lintFile);
  if(!util.exists(configPath)) {
    configPath = path.join(__dirname, `../../${lintFile}`);
  }
  return configPath;
}
...
function parse(processor, filepath, outputPath, config) {
  let extention = util.getExtention(filepath);
  let configPath = lintConfigPath(extention);
